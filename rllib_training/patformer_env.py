...
class PlatformerAgent(MultiAgentEnv, TaskSettableEnv):
    ...
    def __init__(self, env_name: str = "PlatformerAgent",
                 file_name: str = None,
                 time_scale: int = 1,
        ...
        while True:
            ...
            worker_id_ = PlatformerAgent._WORKER_ID if file_name else 0
            PlatformerAgent._WORKER_ID += 1
            ...
            if self.api_version[0] > 1 or (
                ...
                for agent_id in self.unity_env.get_steps(behavior_name)[0].agent_id:
                    ...
                    if key not in action_dict:
                        action_dict[key] = np.zeros(5, dtype=np.float32)
    ...
    def _get_step_results(self):
        ...
        # Force episode termination at horizon
        # if self.episode_timesteps >= 600:
        #     print("manual reset")
        ...
        all_done = True in list(dones.values())
        if all_done:
            print("Done!")
        for agent in dones:
    ...
    def get_policy_configs_for_game(
        ...
        action_spaces = {
            "PlatformerAgent": Box(low=-1.0, high=1.0, shape=(5,)),
        }

        policies = {
            "PlatformerAgent0": PolicySpec(
                observation_space=obs_spaces[game_name],
                action_space=action_spaces[game_name],
            ),
            "PlatformerAgent1": PolicySpec(
                observation_space=obs_spaces[game_name],
                action_space=action_spaces[game_name],
            ),
        }
        ...
        def policy_mapping_fn(agent_id, episode, worker, **kwargs):
            return "PlatformerAgent0" if "team=0" in agent_id else "PlatformerAgent1"

        return policies, policy_mapping_fn

    @staticmethod
    def get_policy_name():
        return "PlatformerAgent"
