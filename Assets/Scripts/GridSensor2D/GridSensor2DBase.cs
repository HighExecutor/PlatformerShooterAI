...
public class GridSensor2DBase : ISensor
{
    string m_Name;
    Vector3 m_CellScale;
    ...
    public SquareOverlapChecker m_BoxOverlapChecker;
    int m_teamId;
    ...
    )
    {
        ...
        if (m_GridSize.y != 1)
        {
            Debug.Log("GridSensor only supports 2D grids.");
        }
    ...
    protected virtual void GetObjectData(GameObject detectedObject, int tagIndex, float[] dataBuffer)
    {
        ...
        if (tagIndex == 1)
        {
            if (detectedObject.GetComponent<CharacterStatus>().TeamId != m_teamId)
            {
                dataBuffer[1] = 1;
        ...
        if (tagIndex == 2)
        {
            ProjectileScript ps = detectedObject.GetComponent<ProjectileScript>();
            if (ps.GetTeamId() != m_teamId || ps is RocketProjectileScript)
            {
                dataBuffer[4] = 1;
                Rigidbody2D rb = detectedObject.GetComponent<Rigidbody2D>();
    ...
    public virtual int GetCellObservationSize()
    {
        return 8;
    }
    ...
    public virtual ProcessCollidersMethod GetProcessCollidersMethod()
    {
    ...
    void ValidateValues(float[] dataValues, GameObject detectedObject)
    {
        ...
        for (int j = 0; j < dataValues.Length; j++)
        {
            if (dataValues[j] < 0 || dataValues[j] > 1)
                Debug.Log(
                    $"When using compression type {m_CompressionType} the data value has to be normalized between 0-1. " +
    ...
    /// </summary>
    public void ProcessDetectedObject(GameObject detectedObject, int cellIndex)
    {
        ...
        for (var i = 0; i < m_DetectableTags.Length; i++)
        {
            if (!ReferenceEquals(detectedObject, null) && detectedObject.CompareTag(m_DetectableTags[i]))
            {
                ...
                {
                    Array.Copy(m_PerceptionBuffer, cellIndex * m_CellObservationSize, m_CellDataBuffer, 0,
                        m_CellObservationSize);
                }
                else
                {
                    Array.Clear(m_CellDataBuffer, 0, m_CellDataBuffer.Length);
                ...
                Array.Copy(m_CellDataBuffer, 0, m_PerceptionBuffer, cellIndex * m_CellObservationSize,
                    m_CellObservationSize);
                break;
            }
        }
