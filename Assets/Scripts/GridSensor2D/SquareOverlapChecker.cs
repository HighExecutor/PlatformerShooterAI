...
public class SquareOverlapChecker
{
    Vector3 m_CellScale;
...
// #endif
        ...
        int maxColliderBufferSize)
    {
        ...
        m_HalfCellScale = new Vector3(cellScale.x, cellScale.y, cellScale.z);
...
// #if MLA_UNITY_PHYSICS_MODULE
        for (var cellIndex = 0; cellIndex < m_NumCells; cellIndex++)
        {
            ...
            ParseCollidersAll(m_ColliderBuffer, numFound, cellIndex, cellCenter, GridOverlapDetectedDebug);
        }
// #endif
    }
...
// #if MLA_UNITY_PHYSICS_MODULE
    ...
    int BufferResizingOverlapBoxNonAlloc(Vector3 cellCenter, Vector3 halfCellScale, Quaternion rotation)
    {
        ...
        {
            numFound =
                Physics2D.OverlapBoxNonAlloc(cellCenter, halfCellScale, rotation.eulerAngles.z, m_ColliderBuffer, m_ColliderMask);
    ...
    void ParseCollidersClosest(Collider2D[] foundColliders, int numFound, int cellIndex, Vector3 cellCenter,
        Action<GameObject, int> detectedAction)
    {
        ...
        for (var i = 0; i < numFound; i++)
        {
            ...
            Vector3 closestColliderPoint = foundColliders[i].ClosestPoint(cellCenter);
    ...
    /// </summary>
    void ParseCollidersAll(Collider2D[] foundColliders, int numFound, int cellIndex, Vector3 cellCenter,
        Action<GameObject, int> detectedAction)
    {
